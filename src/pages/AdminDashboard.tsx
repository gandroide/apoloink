...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [currentView, searchTerm, userFilter]);
  ...
  const loadGlobalData = async () => {
    const { data: studiosData } = await supabase
      .from('studios')
      .select(`*, studio_members ( profiles ( currency ) )`)
  ...
  const handleToggleStudioStatus = async () => {
    ...
    const { error } = await supabase.from('studios').update({ is_active: newStatus }).eq('id', selectedStudio.id);
  ...
  const requestDeleteStudio = () => { setShowDeleteModal(true); };

  const executeDeleteStudio = async () => {
    if (!selectedStudio) return;
    setIsUpdating(true);
    const { error } = await supabase.rpc('delete_studio_completely', { target_studio_id: selectedStudio.id });
    if (error) {
  ...
  const executePromoteUser = async () => {
    ...
    setIsUpdating(true);
    const { error } = await supabase.from('profiles').update({ is_super_admin: true, studio_id: null }).eq('id', userToPromote.id);
    if (error) {
        alert("Error: " + error.message);
    } else {
  ...
  if (userFilter === 'owner') {
      ...
      filteredUsers = filteredUsers.filter(u => u.type === 'independent');
  }
  ...
  return (
    <div className="flex h-screen bg-black text-zinc-300 font-sans overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-8 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 shrink-0">
             <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-lg relative overflow-hidden group">
                ...
                <p className="text-2xl font-mono font-bold text-emerald-400">{format(metrics.mrr)}</p>
            ...
            <div className="flex justify-between items-center p-4 border-b border-zinc-800 bg-zinc-900/40 shrink-0">
                ...
                {currentView === 'users' && (
                    <div className="flex bg-zinc-950 p-1 rounded-lg border border-zinc-800">
                        ...
                        <button onClick={() => setUserFilter('owner')} className={`px-3 py-1 rounded text-[10px] font-bold uppercase transition-all ${userFilter === 'owner' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Dueños</button>
                        <button onClick={() => setUserFilter('independent')} className={`px-3 py-1 rounded text-[10px] font-bold uppercase transition-all ${userFilter === 'independent' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Independientes</button>
                    </div>
                )}
            </div>
                    ...
                    <tbody className="text-xs">
                        {currentItems.map((item, index) => {
                            ...
                            if (currentView === 'studios') {
                                const studio = item;
                                const owner = users.find(u => u.studio_id === studio.id && u.type === 'owner');
                                const studioCurrency = owner ? owner.currency : 'COP';
                                ...
                                return (
                                    <tr key={studio.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/40 transition-colors">
                                        ...
                                        <td className="py-3 px-4 text-[10px] font-bold text-zinc-400">
                                            {getCurrencyFlag(studioCurrency)}
                                        </td>
                                        ...
                                        <td className="py-3 px-4 font-mono text-zinc-400">{format(studio.subscription_price || 0)}</td>
                                ...
                                return (
                                    <tr key={user.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/40 transition-colors">
                                        ...
                                        <td className="py-3 px-4 text-[10px] font-bold text-zinc-400">
                                            {getCurrencyFlag(user.currency)}
                                        </td>
                                        <td className="py-3 px-4 font-mono text-zinc-400 text-[11px]">{formatDate(user.created_at)}</td>
                                        <td className="py-3 px-4">
                                            {user.is_super_admin 
                                                ? <span className="text-purple-400 font-black uppercase text-[9px]">👑 Super Admin</span> 
                                                : user.type === 'independent'
                                                    ? <span className="text-emerald-400 font-black uppercase text-[9px]">Independiente</span>
             ...
             <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                ...
                <div className="space-y-4 overflow-y-auto pr-2">
                    <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-800 space-y-3">
                        <p className="text-[10px] uppercase font-bold text-zinc-500 mb-2 border-b border-zinc-800 pb-1">Datos de Contacto</p>
                        <div className="flex items-center gap-3"><div className="text-zinc-400"><Icons.Mail /></div><p className="font-mono text-xs text-white break-all">{selectedStudio.contact_email || 'No registrado'}</p></div>
                        <div className="flex items-center gap-3"><div className="text-zinc-400"><Icons.Phone /></div><p className="font-mono text-xs text-white">{selectedStudio.phone || 'No registrado'}</p></div>
                        <div className="flex items-center gap-3"><div className="text-zinc-400"><Icons.MapPin /></div><p className="text-xs text-white">{selectedStudio.address || 'No registrado'}</p></div>
                    </div>
                    <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-800 flex justify-between items-center"><div><p className="text-[10px] uppercase font-bold text-zinc-500">Valor de Suscripción</p><p className="font-mono text-xl font-bold text-white">{format(selectedStudio.subscription_price || 0)}<span className="text-xs text-zinc-500 font-normal">/mes</span></p></div></div>
                    <button onClick={handleToggleStudioStatus} disabled={isUpdating} className={`w-full py-3 rounded-lg font-bold uppercase text-xs tracking-widest transition-all ${selectedStudio.is_active !== false ? 'bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-700' : 'bg-emerald-950 text-emerald-500 border border-emerald-900/50 hover:bg-emerald-900 hover:text-white'}`}>{isUpdating ? 'Procesando...' : (selectedStudio.is_active !== false ? 'SUSPENDER SERVICIO' : 'REACTIVAR SERVICIO')}</button>
                    <div className="pt-6 mt-6 border-t border-zinc-800/50"><button onClick={requestDeleteStudio} className="w-full flex items-center justify-center gap-2 py-3 rounded-lg font-bold uppercase text-xs tracking-widest transition-all bg-red-950/30 text-red-500 border border-red-900/30 hover:bg-red-900 hover:text-white"><Icons.Trash /> Eliminar Estudio</button></div>
                </div>
            </div>
        </div>
      )}

      {selectedUser && !showPromoteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
             <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                {/* CONTENIDO DEL MODAL USUARIO (Igual al anterior) */}
                <div className="flex justify-between items-start mb-6"><div><h3 className="text-lg font-bold text-white">Ficha de Usuario</h3><p className="text-xs text-zinc-500 mt-1">Detalles de registro</p></div><button onClick={() => setSelectedUser(null)} className="text-zinc-500 hover:text-white"><Icons.Close /></button></div>
                <div className="space-y-4">
                    <div className="flex items-center gap-4 mb-6"><div className="h-16 w-16 bg-zinc-800 rounded-full flex items-center justify-center text-2xl font-black text-white uppercase">{selectedUser.name ? selectedUser.name.substring(0,2) : 'US'}</div><div><h2 className="text-xl font-black text-white uppercase italic">{selectedUser.name || 'Sin Nombre'}</h2><p className="text-zinc-500 text-xs font-mono">{selectedUser.type === 'owner' ? 'DUEÑO DE ESTUDIO' : 'ARTISTA INDEPENDIENTE'}</p></div></div>
                    <div className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 space-y-4">
                        <div className="flex items-center justify-between border-b border-zinc-800 pb-2"><span className="text-[10px] font-bold text-zinc-500 uppercase flex items-center gap-2"><Icons.Globe /> Moneda</span><span className="text-white font-bold text-sm">{getCurrencyFlag(selectedUser.currency)}</span></div>
                        <div className="flex items-center justify-between border-b border-zinc-800 pb-2"><span className="text-[10px] font-bold text-zinc-500 uppercase flex items-center gap-2"><Icons.Mail /> Email</span><span className="text-white font-mono text-xs">{selectedUser.email}</span></div>
                        <div className="flex items-center justify-between border-b border-zinc-800 pb-2"><span className="text-[10px] font-bold text-zinc-500 uppercase flex items-center gap-2"><Icons.Phone /> Teléfono</span><span className="text-white font-mono text-xs">{selectedUser.phone || '---'}</span></div>
                        <div className="flex flex-col gap-1"><span className="text-[10px] font-bold text-zinc-500 uppercase flex items-center gap-2"><Icons.MapPin /> Dirección</span><span className="text-white text-xs">{selectedUser.address || 'No registrada'}</span></div>
                    </div>
                    <div className="text-center pt-2"><p className="text-[9px] text-zinc-600 font-mono">ID: {selectedUser.id}</p></div>
                </div>
             </div>
        </div>
      )}
      ...
      {showNewAdminModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                <div className="flex justify-between items-start mb-4"><h3 className="text-lg font-bold text-white flex items-center gap-2"><Icons.UserPlus /> Nuevo Super Admin</h3><button onClick={() => setShowNewAdminModal(false)} className="text-zinc-500 hover:text-white"><Icons.Close /></button></div>
      ...
      {showPromoteModal && userToPromote && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-zinc-950 border-2 border-yellow-600 rounded-2xl w-full max-w-sm p-6 shadow-[0_0_50px_rgba(234,179,8,0.2)] animate-in zoom-in-95 duration-200 text-center relative overflow-hidden">
                ...
                <div className="mx-auto h-16 w-16 bg-yellow-900/30 border border-yellow-700/50 rounded-full flex items-center justify-center mb-4 text-yellow-500 animate-bounce relative z-10"><div className="scale-150"><Icons.Crown /></div></div>
                <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2 relative z-10">¿Otorgar Super Poderes?</h3>
                <p className="text-xs text-zinc-400 mb-6 leading-relaxed relative z-10">Estás a punto de ascender a <strong className="text-white">{userToPromote.name || userToPromote.email}</strong>.<br /><br />Tendrá acceso total a <strong>AXIS.hq</strong>, podrá ver ingresos, borrar estudios y gestionar todo el sistema.</p>
                <div className="flex flex-col gap-3 relative z-10"><button onClick={executePromoteUser} disabled={isUpdating} className="w-full py-3 bg-yellow-600 hover:bg-yellow-500 text-black font-black uppercase rounded-lg tracking-widest transition-all shadow-lg shadow-yellow-900/20 hover:scale-[1.02] active:scale-95">{isUpdating ? 'Procesando...' : 'Sí, Ascender Usuario'}</button><button onClick={() => setShowPromoteModal(false)} disabled={isUpdating} className="w-full py-3 bg-transparent hover:bg-zinc-900 text-zinc-500 hover:text-white font-bold uppercase rounded-lg tracking-widest transition-all">Cancelar</button></div>
            </div>
        </div>
      )}
