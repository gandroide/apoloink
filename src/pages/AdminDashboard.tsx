...
export const AdminDashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-black text-zinc-300 font-sans overflow-hidden">
      ...
      <aside className="w-64 border-r border-zinc-800 bg-zinc-950 flex flex-col justify-between shrink-0">
        <div>
          ...
          <nav className="p-4 space-y-1">
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-colors ${currentView === 'studios' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-white hover:bg-zinc-900'}`}
            >
              <Icons.Building /> Estudios
            </button>
            <button 
              onClick={() => setCurrentView('users')}
              className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-colors ${currentView === 'users' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-white hover:bg-zinc-900'}`}
            >
        ...
        <div className="p-4 border-t border-zinc-800">
          <button 
            ...
            className="w-full flex items-center gap-3 px-4 py-2.5 text-red-500 hover:bg-red-950/30 rounded-lg text-xs font-bold uppercase tracking-wide transition-colors"
          >
            <Icons.Exit /> Cerrar Sesión
          </button>
        </div>
      </aside>
        ...
        <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-8 bg-black shrink-0">
          <div className="flex items-center gap-4">
              <h2 className="text-sm font-bold text-white uppercase tracking-widest">
                {currentView === 'studios' ? 'Gestión de Estudios' : 'Directorio de Usuarios'}
              </h2>
              ...
              {currentView === 'users' && (
                  <button 
                    ...
                    className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-white text-[10px] font-bold uppercase px-3 py-1.5 rounded-lg border border-zinc-700 transition-all"
                  >
                      <Icons.UserPlus /> Nuevo Admin
                  </button>
              )}
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
                <input 
                    type="text" 
                    placeholder={currentView === 'studios' ? "Buscar estudio..." : "Buscar nombre o email..."}
                    className="bg-zinc-900 border border-zinc-800 text-xs text-white pl-9 pr-4 py-2 rounded-lg outline-none focus:border-zinc-600 w-64 transition-all"
                    value={searchTerm}
        ...
        <div className="flex-1 overflow-auto p-8 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 shrink-0">
            <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-lg relative overflow-hidden group">
                <div className="absolute right-4 top-4 text-zinc-800 group-hover:text-emerald-900/40 transition-colors">
                    <Icons.Money />
                </div>
            ...
            <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-lg">
                <p className="text-[10px] uppercase font-bold text-zinc-500 mb-1">Estudios Totales</p>
                <p className="text-2xl font-mono font-bold text-white">{studios.length}</p>
            </div>
            <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-lg">
                <p className="text-[10px] uppercase font-bold text-zinc-500 mb-1">Usuarios Totales</p>
                <p className="text-2xl font-mono font-bold text-white">{users.length}</p>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/20 border border-zinc-800 rounded-lg flex flex-col flex-1 min-h-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex justify-between items-center p-4 border-b border-zinc-800 bg-zinc-900/40 shrink-0">
                <h3 className="text-xs font-bold text-white uppercase tracking-wider">
                   {currentView === 'studios' ? 'Listado de Estudios' : 'Listado de Usuarios'}
                </h3>
            </div>
            ...
            <div className="overflow-auto flex-1">
                                ...
                                return (
                                    <tr key={studio.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/40 transition-colors">
                                        ...
                                        <td className="py-3 px-4">
                                            {studio.contact_email ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="font-mono text-zinc-300">{studio.contact_email}</span>
                                                </div>
                                            ) : (
                                        ...
                                        <td className="py-3 px-4 font-mono text-zinc-400">
                                            {formatterCOP.format(studio.subscription_price || 0)}
                                        </td>
                                        <td className="py-3 px-4">
                                            <span className={`inline-block px-2 py-0.5 rounded border text-[9px] font-bold uppercase ${studio.is_active !== false ? 'bg-emerald-950/50 text-emerald-500 border-emerald-900/30' : 'bg-red-950/50 text-red-500 border-red-900/30'}`}>
                                                {studio.is_active !== false ? 'Activo' : 'Suspendido'}
                                            </span>
                                        </td>
                                        <td className="py-3 px-4 text-right">
                                            <button 
                                                onClick={() => setSelectedStudio(studio)}
                                                className="text-zinc-400 hover:text-white font-bold uppercase text-[9px] border border-zinc-700 px-3 py-1.5 rounded hover:bg-zinc-800 transition-all"
                                            >
                                                Gestionar
                                            </button>
                                        </td>
                                ...
                                return (
                                    <tr key={user.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/40 transition-colors">
                                        ...
                                        <td className="py-3 px-4">
                                            ...
                                            ) : <span className="text-zinc-600 italic">No disponible</span>}
                                        </td>
                                        ...
                                        <td className="py-3 px-4">
                                            {user.is_super_admin ? (
                                                ...
                                                <span className="text-zinc-500 font-mono uppercase text-[9px]">{user.type || 'Usuario'}</span>
                                            )}
                                        </td>
                                        ...
                                        <td className="py-3 px-4 text-right">
                                            {!user.is_super_admin ? (
                                                ...
                                                <div className="flex items-center justify-end gap-2">
                                                    <span className="text-zinc-500 text-[9px] font-black uppercase flex items-center gap-1 border border-zinc-800 px-2 py-1 rounded bg-zinc-900">
                                                        <Icons.CheckBadge /> Verificado
                                                    </span>
                                                </div>
                                            )}
                                        </td>
                ...
                {currentItems.length === 0 && (
                     <div className="p-8 text-center text-zinc-600 text-xs uppercase font-bold">No hay datos para mostrar.</div>
                )}
            </div>

            {/* PAGINATION */}
            <div className="p-3 border-t border-zinc-800 bg-zinc-900/40 flex justify-between items-center shrink-0">
                <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wide">Mostrando {currentItems.length > 0 ? indexOfFirstItem + 1 : 0} - {Math.min(indexOfLastItem, currentData.length)} de {currentData.length}</span>
                <div className="flex items-center gap-2">
            ...
            <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="space-y-4">
                    ...
                    <button 
                        ...
                        disabled={isUpdating}
                        className={`w-full py-3 rounded-lg font-bold uppercase text-xs tracking-widest transition-all ${selectedStudio.is_active !== false ? 'bg-red-950 text-red-500 border border-red-900/50' : 'bg-emerald-950 text-emerald-500 border border-emerald-900/50'}`}
                    >
      ...
      {/* MODAL INSTRUCCIONES NUEVO ADMIN */}
      {showNewAdminModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="space-y-4 text-xs text-zinc-400">
                    ...
                    <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-800 space-y-3">
                        <div className="flex gap-3">
                            <span className="bg-zinc-800 h-5 w-5 rounded-full flex items-center justify-center text-white font-bold text-[10px]">1</span>
                            <p>Pídele a tu socio que se registre normalmente en la App <span className="text-white">(/signup)</span>.</p>
                        </div>
                        <div className="flex gap-3">
                            <span className="bg-zinc-800 h-5 w-5 rounded-full flex items-center justify-center text-white font-bold text-[10px]">2</span>
                            <p>Una vez registrado, búscalo aquí en la lista de <span className="text-white font-bold uppercase">Usuarios</span>.</p>
                        </div>
                        <div className="flex gap-3">
                            <span className="bg-zinc-800 h-5 w-5 rounded-full flex items-center justify-center text-white font-bold text-[10px]">3</span>
                            <p>Haz clic en el botón dorado <span className="text-yellow-500 font-bold uppercase">Ascender</span>.</p>
                        </div>
                    </div>
