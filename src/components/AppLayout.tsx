...
export const AppLayout = ({ children }: LayoutProps) => {
  const [showUserMenu, setShowUserMenu] = useState(false); // Controla el menú de PC
  const [showMobileMenu, setShowMobileMenu] = useState(false); // Controla el menú de Móvil
  ...
  return (
    <div className="flex min-h-screen bg-[var(--brand-bg)] text-[var(--brand-primary)] font-sans">
      ...
      <aside className="hidden lg:flex flex-col w-72 border-r border-[var(--brand-border)] fixed h-full bg-[var(--brand-surface)] z-50 p-6">
        ...
        {/* Footer Sidebar (con Menú hacia ARRIBA) */}
        <div className="mt-auto pt-6 border-t border-[var(--brand-border)] space-y-4 relative">
            
            {showUserMenu && (
              <div className="absolute bottom-[calc(100%-10px)] left-0 right-0 bg-[#0a0a0a] border border-[var(--brand-border)] rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in slide-in-from-bottom-2 z-50 mb-4">
                <button 
                  onClick={() => { setShowUserMenu(false); navigate('/settings'); }}
                  className="w-full text-left px-4 py-3 hover:bg-[var(--brand-bg)] flex items-center gap-3 transition-colors group"
                >
                  <div className="text-[var(--brand-text-muted)] group-hover:text-[var(--brand-accent)] transition-colors">
                ...
                >
                  <div className="text-[var(--brand-text-muted)] group-hover:text-red-400 transition-colors">
                     <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>
                  </div>
                  <span className="text-xs font-bold text-red-400 group-hover:text-red-300">Cerrar Sesión</span>
                </button>
              </div>
            )}
             ...
             <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-[var(--brand-accent)] text-black py-4 rounded-2xl hover:opacity-90 transition-all active:scale-95 shadow-[0_0_15px_var(--brand-accent)] z-0"
              >
            ...
            >
                <div className={`w-10 h-10 rounded-full border flex items-center justify-center transition-colors overflow-hidden ${showUserMenu ? 'bg-[var(--brand-bg)] border-[var(--brand-accent)] text-[var(--brand-accent)]' : 'bg-[var(--brand-border)] border-[var(--brand-border)] text-[var(--brand-text-muted)] group-hover:border-[var(--brand-primary)]'}`}>
                ...
                <div className="overflow-hidden flex-1 text-left">
                    ...
                    <p className="text-[9px] text-[var(--brand-text-muted)] truncate flex items-center gap-1">
                      Admin
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 bg-[var(--brand-surface)]/95 backdrop-blur-xl border-b border-[var(--brand-border)] z-40 px-4 py-3 flex justify-between items-center">
        ...
        {/* ÁREA DE USUARIO MÓVIL (Con Dropdown hacia ABAJO) */}
        <div className="relative">
          <button 
              onClick={() => setShowMobileMenu(!showMobileMenu)}
              className={`w-8 h-8 rounded-full border flex items-center justify-center transition-colors ${showMobileMenu ? 'border-[var(--brand-accent)] text-[var(--brand-accent)]' : 'bg-[var(--brand-border)] border-transparent text-[var(--brand-text-muted)]'}`}
          >
               <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
          </button>

          {/* MENÚ FLOTANTE MÓVIL (Absolute Right-0) */}
          {showMobileMenu && (
            <div className="absolute top-full right-0 mt-3 w-48 bg-[#0a0a0a] border border-[var(--brand-border)] rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 z-50">
               <button 
                  onClick={() => { setShowMobileMenu(false); navigate('/settings'); }}
                  className="w-full text-left px-4 py-3 hover:bg-[var(--brand-bg)] flex items-center gap-3 transition-colors border-b border-[var(--brand-border)]"
                >
                ...
                >
                  <svg className="text-red-400" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>
