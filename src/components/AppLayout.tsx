...
export const AppLayout = ({ children }: LayoutProps) => {
  ...
  // 1. DETECTAR EL ROL AL CARGAR EL LAYOUT
  useEffect(() => {
    const getRole = async () => {
      if (!user) return;
      ...
      try {
        ...
        console.log("Perfil detectado en DB:", data); // <--- DEBUG: Mira esto en la consola (F12)

        // LÓGICA MÁS ESTRICTA:
        // Solo si dice explícitamente 'owner' eres dueño.
        // Todo lo demás (null, 'residente', 'independent', vacío) es Artista.
        if (data?.type === 'owner') {
          setUserRole('owner');
        } else {
        ...
        setUserRole('independent'); // En caso de duda, restringir acceso
      }
    };
  ...
  // 2. FILTRADO (Lógica Invertida: Ocultar por defecto)
  const visibleNavItems = navItems.filter(item => {
    // Si el ítem es 'Artistas' (/team), SOLO MOSTRAR si userRole es 'owner'
    if (item.path === '/team') {
    ...
    <div className="flex min-h-screen bg-[var(--brand-bg)] text-[var(--brand-primary)] font-sans">
        ...
        <div className="mt-auto pt-6 border-t border-[var(--brand-border)] space-y-4 relative">
            {showUserMenu && (
              <div className="absolute bottom-[calc(100%-10px)] left-0 right-0 bg-[var(--brand-surface)] border border-[var(--brand-border)] rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] overflow-hidden animate-in fade-in slide-in-from-bottom-2 z-50 mb-4">
                <button onClick={() => { setShowUserMenu(false); navigate('/settings'); }} className="w-full text-left px-4 py-3 hover:bg-[var(--brand-primary)]/5 flex items-center gap-3 transition-colors group">
                ...
                <button onClick={handleLogout} className="w-full text-left px-4 py-3 hover:bg-red-500/10 flex items-center gap-3 transition-colors group">
             ...
             <button onClick={() => navigate('/scan')} className="w-full flex items-center justify-center gap-2 bg-[var(--brand-accent)] text-[var(--brand-surface)] py-4 rounded-2xl hover:opacity-90 transition-all active:scale-95 shadow-[0_0_15px_var(--brand-accent)] z-0">
            ...
            <button onClick={() => setShowUserMenu(!showUserMenu)} className={`w-full flex items-center gap-3 px-2 cursor-pointer p-2 rounded-xl transition-all group ${showUserMenu ? 'bg-[var(--brand-bg)]' : 'hover:bg-[var(--brand-bg)]'}`}>
                ...
                <div className="overflow-hidden flex-1 text-left">
                    <p className="text-[10px] font-black uppercase text-[var(--brand-primary)] tracking-wider truncate">{user?.email?.split('@')[0] || 'Mi Cuenta'}</p>
                    <p className="text-[9px] text-[var(--brand-primary)] opacity-60 truncate flex items-center gap-1">
                      {userRole === 'independent' ? 'Artista' : 'Admin'} 
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 bg-[var(--brand-surface)]/95 backdrop-blur-xl border-b border-[var(--brand-border)] z-40 px-4 py-3 flex justify-between items-center">
        ...
        <div className="relative">
          <button onClick={() => setShowMobileMenu(!showMobileMenu)} className={`w-8 h-8 rounded-full border flex items-center justify-center transition-colors ${showMobileMenu ? 'border-[var(--brand-accent)] text-[var(--brand-accent)]' : 'bg-[var(--brand-border)] border-transparent text-[var(--brand-primary)] opacity-60'}`}>
          ...
          {showMobileMenu && (
            <div className="absolute top-full right-0 mt-3 w-48 bg-[var(--brand-surface)] border border-[var(--brand-border)] rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 z-50">
               <button onClick={() => { setShowMobileMenu(false); navigate('/settings'); }} className="w-full text-left px-4 py-3 hover:bg-[var(--brand-primary)]/5 flex items-center gap-3 transition-colors border-b border-[var(--brand-border)]">
                ...
                <button onClick={handleLogout} className="w-full text-left px-4 py-3 hover:bg-red-500/10 flex items-center gap-3 transition-colors">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[var(--brand-surface)]/95 backdrop-blur-xl border-t border-[var(--brand-border)] z-50 px-2 py-3 pb-8 safe-area-bottom">
        <div className="flex justify-between items-center max-w-md mx-auto relative">
          ...
          {/* CORRECCIÓN FINAL: Usamos visibleNavItems */}
          {visibleNavItems.map((item) => {
            const isActive = item.path === '/' ? location.pathname === '/' : location.pathname.startsWith(item.path);
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 transition-all flex-1 min-w-0 ${
                  isActive ? 'text-[var(--brand-primary)] scale-105' : 'text-[var(--brand-primary)] opacity-60'
                }`}
              >
